...
class Cards extends Component {
    ...
    loadData() {
        ...
        this.checkFilterHandler(data);
    }

    checkFilterHandler(data) {
        const filteredList = data.filter( (item) => {
            return item.tags[0] === this.state.params;
        });
        this.setState({data: filteredList});
        console.log("params Cards.js: ", this.state.params)
    }

    renderPlantsHandler() {
        const plants = this.state.data.map((plant, index) => 
